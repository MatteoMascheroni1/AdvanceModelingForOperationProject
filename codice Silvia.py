# -*- coding: utf-8 -*-
"""
Created on Mon Oct 17 19:10:05 2022
...
def read_line_info(): #Function that imports the coordinates of the lines output points from a file
    ...
    with open(path, "r") as f:
        ...
        for line in f.readlines()[1:]:
            a = line.split(',') #The elements of the list a are strings, corresponding to the comma-separated values in the line ["0,1,2,3,4", "....", "....."]
            x.append(float(a[1]))
            y.append(float(a[2]))
            cycle_times.append(float(a[3])) 
            weights.append(float(a[4]))
    return(x, y,cycle_times, weights) #Tupla
...
n_shifts = 2 #number of shifts per day 
wh_per_shift = 8 #working hours per shifts 
...
def compute_time(distance): #Function that computes the time needed to drive a certain distance:
    time = (distance/speed) + random.uniform(0,1) #random.uniform(0,1) to account for possible stops due to the presence of obstacles on the vehicle path (e.g., people, unit loads not placed correctly, ..)
    return  time #[seconds] --> distance [m]/speed[m/s]
...
class Train(Agent):
    def __init__(self, unique_id, model):
        ...
        self.battery_size = battery_size # [kWh] #CHECK
        ...
        self.weight = 0 #current weight of tugger train
        self.totweight = 2000 #[ton] total weight of the transported unit load
    ...
    def move(self): #Function that simulates the travel of the tugger train to the next stop and the loading/unloading of unit loads:
            ...
            if (self.pos_x,self.pos_y) != (warehouse_coord[0],warehouse_coord[1]): #If the reached position is a line output point (and not the warehouse)
                if self.model.schedule_lines.agents[self.next_line].UL_in_buffer >= 1: #If there is at least one unit load at the line output point
                    if self.load < self.capacity: # If the train is not full, it loads one unit load
                        if self.next_line == 1: #SONO SICURA SI POSSA SCRIVERE MEGLIO DI COSI
                                self.weight = self.weight + 400
                        elif self.next_line == 2:
                                self.weight = self.weight + 600
                        elif self.next_line == 3:
                                self.weight = self.weight + 700
                        elif self.next_line == 4:
                                self.weight = self.weight + 500
                        elif self.next_line == 4:
                                self.weight = self.weight + 500 #sono un idiota ho salvato i valori dal file-->rifare in modo più sintetico
                            
                        
                        if self.weight < self.totweight:
                            ...
                            print("\n"+self.unique_id,"going to line",self.next_line,"- Not enough weight capacity left\nweight carried:", self.weight)
                    else:
        ...
        print("Travelled distance:",distance_next_stop,"m","- Task endtime (minutes):",round(self.task_endtime,2),"- Remaining energy:",round(self.remaining_energy,2),"kWh")
    ...
    def charging(self): #Function that simulates the (possible) queuing at the charging station and the battery charging:
        ...
        #Queueing time (waiting for the charging station to be available):
        ...
        print("Task endtime (hours):",round(self.task_endtime/3600,2),"- \nRemaining energy:",self.remaining_energy, "kWh", "capacity", self.capacity, "load", self.load)
        
        self.next_line = 0 #Called only according to the step() function, thus when the first if statement is true
    ...
    def step(self):
        if self.task_endtime <= self.model.system_time:
            ...
            if self.need_to_charge == True:
                ...
                self.move()
    print(battery_size, "%d")
    print(charging_stations_x)
    print(charging_stations_y)
...
class FactoryModel(Model):
    def __init__(self): 
        ...
        self.system_time = 1 #Changed from 0 to 1 #This attribute will keep track of the system time, advancing by 1 minute at each step
    ...
    def step(self):
        ...
        #Maximum number of unit loads which can be loaded on a tugger train
